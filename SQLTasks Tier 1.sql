...
SELECT b.bookid, b.starttime, f.name AS facility_name, CONCAT(m.firstname, ' ', m.surname) AS member_name, 
	CASE WHEN b.memid <> 0 THEN SUM(f.membercost*b.slots)
		 ELSE SUM(f.guestcost*b.slots) END AS total_cost
FROM Bookings AS b
...
SELECT (m1.surname || " " || m1.firstname) AS member_name, (m2.surname || " " || m2.firstname) AS recommended_by
        ...
        WHERE m1.recommendedby NOT NULL;

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT f.name, m.surname, m.firstname, COUNT(m.surname) AS times_used
        FROM Bookings as b
        INNER JOIN Facilities AS f USING (facid)
        INNER JOIN Members AS m USING (memid)
...
SELECT month, facility, COUNT(facility) AS facility_usage
        FROM (SELECT strftime('%m', starttime) AS month, f.name AS facility
                FROM Bookings as b
                INNER JOIN Facilities as f USING(facid)
                INNER JOIN Members as m USING(memid)
                WHERE memid <> 0) AS s
        GROUP BY month, facility
        ORDER BY month, facility_usage DESC;
